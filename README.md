...
def _get_embedding(self, text: str) -> List[float]:
    ...
    return embedding.tolist()
```
<br>Далее просто в функциях поиска и индексации заменить query_embedding или embedding на соответствующие им переменные с или без флага is_query, например:<br>
```python
query_embedding = self._get_embedding(query, is_query=True) -> query_embedding = self._get_embedding(query)
```
