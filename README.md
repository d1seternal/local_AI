...
* **tests** – папка, содержащая небольшие unit-тесты некоторых функций на базе pytest + скрипты для сравнения квантизаций выбранной в проекте llm-модели. Ввиду небольших ресурсов RAM и использования лишь мощностей процессора для упрощенного локального запуска приложения скрипт содержит Q3-, Q4-, Q5-квантизации для наглядного сравнения качества ответов, скорости ответов, времени загрузки и объемов используемой памяти.

<br>Структура файлов в пятом блоке:<br>

- quantization: папка для тестирования разных квантизаций выбранной модели.
  - quantization_benchmark.py: модуль для бенчмаркинга моделейй
  - run_benchmarks.py: модуль запуска бенчмаркинга
- test_shared: папка unit-тестов различных модулей проекта.
  - test_document_parser.py
  - test_main_llm_rag.py
  - test_memory.py
  - test_prompts.py
- conftest.py: специальный модуль для pytest, который содержит общие фикстуры для всех тестов в проекте.
