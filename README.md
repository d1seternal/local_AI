...
conda==26.1.1<br>cmake==3.29.5-msvc4<br>llama-cpp-python==0.3.16<br>torch==2.10.0<br>chromadb==1.5.2<br>sentence-transformers==5.2.3<br>PyPDF2==3.0.1<br>python-docx==1.2.0<br>python==3.11.4<br>huggingface-hub==0.36.2<br>docling[complete]==2.77.0<br>pandas==2.3.3<br>tabulate==0.9.0<br>langchain==1.2.11
## Структура проекта:
```mermaid
graph TD
    subgraph "local_AI"
...
- RAG-ассистент (main_llm_rag, memory, propmpts, document_parser, reranker) подключен к блоку, чтобы агент мог задейстовать ассистента для выполнения запроса пользователя. Все файлы импортированы в данный блок, чтобы впоследствии была возможность расширять функционал агента, который сможет подключать RAG не только для поиска информации и генерации ответа.
- agent.py: основной модуль с проектированием и запуском агента. Для запуска ReAct-агента используется именно этот файл.
